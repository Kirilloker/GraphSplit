...
{
    public class GraphWithWeight : BaseGraph
    {
        private const int VertexWeight = 1;

        public GraphWithWeight(List<GraphSplit.GraphElements.Vertex> vertices, Random rnd)
        {
        ...
        public override void InitializeGraph()
        {
            ...
            foreach (var my_vert in my_vertices)
            {
                ...
                {
                    connectedEdges.Add(edges.GetOtherVertex(my_vert).Index, (int)edges.weight);
                    number_edges += 1;
                }
